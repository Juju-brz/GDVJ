...
func update_sprites_transform(delta):
	...
	var target_scaleX = lerp(1.0, 1.2, mouse_norm.x) # Mouse X controls size
	var target_scaleY = lerp(0.3, 0.7, mouse_norm.y) # Mouse X controls size
	...
	for i in range(duplicated_spriteslist.size()):
		...
		if is_instance_valid(sprite):
			...
			var x_pos = screen_center.x #+ sin(angle) * current_radius
			...
			sprite.scale = Vector2(target_scaleX, target_scaleY)
